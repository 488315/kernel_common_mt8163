...
static int __init ramoops_memreserve(char *p)
{
	...
	pr_info("mtk_reserve_ramoops_memory addr=%lx, size=%lx\n",
		ramoops_data.mem_address, ramoops_data.mem_size);
	pr_info("mtk_reserve_ramoops_memory record_size=%lx, ftrace_size=%lx\n",
...
static int __init mtk_register_ramoops_device(void)
{
    pr_info("mtk_register_ramoops_device \n");
	if (platform_device_register(&ramoops_dev))
		pr_info("Unable to register ramoops platform device\n");
    return 0;
}
core_initcall(mtk_register_ramoops_device);
